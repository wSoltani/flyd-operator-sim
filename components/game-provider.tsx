...
function calculateDynamicStats(worker: Worker, incidents: Incident[]): { cpu: number; memory: number; disk: number } {
  ...
  const workerIncidents = incidents.filter((i) => i.workerId === worker.id && !i.resolved)
  ...
  return {
    ...
    disk: Math.round(Math.max(worker.baseStats.disk, Math.min(100, disk)) * 10) / 10,
  }
}
...
function gameReducer(state: GameState, action: GameAction): GameState {
  switch (action.type) {
    case "START_GAME":
      return { ...state, gameStarted: true, paused: false }
    ...
    case "TICK":
      ...
      const updatedWorkers = state.workers.map((worker) => {
        ...
        const updatedFSMs = worker.activeFSMs.map((fsm) => {
          ...
          if (fsm.type === "migration") {
            if (elapsed < 10000) {
              ...
              newState = "cloning"
              newProgress = Math.min(30, ((elapsed - 10000) / 15000) * 30)
            } else if (elapsed < 60000) {
              newState = "hydrating"
              newProgress = Math.min(80, 30 + ((elapsed - 25000) / 35000) * 50)
            } else if (elapsed < 70000) {
              newState = "booting_new"
              newProgress = Math.min(95, 80 + ((elapsed - 60000) / 10000) * 15)
            } else if (elapsed < 75000) {
              newState = "running_new"
              newProgress = Math.min(99, 95 + ((elapsed - 70000) / 5000) * 4)
            } else {
          ...
          return { ...fsm, state: newState, progress: Math.round(newProgress * 10) / 10 }
        })
        ...
        const dynamicStats = calculateDynamicStats(updatedWorker, state.incidents)
      ...
      const updatedIncidents = state.incidents.map((incident) => {
        ...
        const affectedWorker = updatedWorkers.find((w) => w.id === incident.workerId)
        ...
        switch (incident.type) {
          ...
          case "migration_stuck":
            ...
            shouldResolve =
              ...
              affectedWorker.activeFSMs.every((fsm) => fsm.state !== "hydrating" || fsm.progress > 80)
            break
          case "network_partition":
          case "network_congestion":
          case "dns_failure":
      ...
      const shouldClearFeedback = state.actionFeedback && Date.now() - state.actionFeedback.timestamp > 10000
    ...
    case "RESTART_FLYD":
      ...
      const resolvedIncidentsAfterRestart = state.incidents.map((incident) => {
        if (
          ...
          !incident.resolved &&
          ["flyd_stalled", "containerd_sync", "memory_leak", "config_corruption"].includes(incident.type)
        ) {
          return { ...incident, resolved: true }
        }
        return incident
    ...
    case "DRAIN_WORKER":
      const drainSuccess = Math.random() > 0.3 // 70% success rate
      ...
      return {
        ...
        actionFeedback: {
          ...
          message:
            "Initiating graceful worker drain. All 12 machines will migrate using dm-clone to other workers. This will take ~75 seconds.",
          timestamp: Date.now(),
        },
      }
    ...
    case "INSPECT_LVM":
      ...
      return {
        ...state,
        actionFeedback: {
          type: diskUsage > 90 ? "error" : diskUsage > 80 ? "warning" : "action",
    ...
    case "INVESTIGATE_INCIDENT":
      ...
      return {
        ...state,
        incidents: state.incidents.map((i) => (i.id === action.incidentId ? { ...i, investigated: true } : i)),
        actionFeedback: {
          type: "investigation",
          title: result.title,
    ...
    case "VIEW_FLYD_LOGS":
      const logIncident = state.incidents.find((i) => i.id === action.incidentId)
      ...
      const logMessages = {
        ...
        storage_corruption: "FATAL [LVM] metadata read failed. VG: fly-volumes. Corrupted LVs: vol-abc123, vol-def456",
        memory_leak:
      ...
      return {
        ...state,
        incidents: state.incidents.map((i) => (i.id === action.incidentId ? { ...i, logViewed: true } : i)),
        actionFeedback: {
          type: "action",
          title: "flyd Logs Retrieved",
          message: logMessages[logIncident.type] || "No relevant errors found in recent logs.",
          timestamp: Date.now(),
        },
      }

    case "FORCE_FSM_TRANSITION":
      const fsmIncident = state.incidents.find((i) => i.id === action.incidentId)
      if (!fsmIncident) return state

      const success = Math.random() > 0.3
      return {
        ...state,
        incidents: state.incidents.map((i) => (i.id === action.incidentId ? { ...i, resolved: success } : i)),
        score: {
          ...state.score,
          riskyActions: state.score.riskyActions + 2,
          failedMigrations: success ? state.score.failedMigrations : state.score.failedMigrations + 1,
        },
        actionFeedback: {
          type: success ? "success" : "error",
          title: success ? "FSM Force Transition Successful" : "FSM Force Transition Failed",
          message: success
    ...
    case "QUICK_FIX_INCIDENT":
      const quickFixIncident = state.incidents.find((i) => i.id === action.incidentId)
      if (!quickFixIncident) return state

      const quickFixes = {
        flyd_stalled: { action: "Restart flyd", success: 0.8, takesTime: true, actionType: "RESTART_FLYD" },
        migration_stuck: { action: "Cancel migration", success: 0.6, takesTime: false },
        containerd_sync: { action: "Restart flyd", success: 0.9, takesTime: true, actionType: "RESTART_FLYD" },
        network_partition: { action: "Restart networking", success: 0.7, takesTime: false },
        storage_corruption: { action: "Mark volumes read-only", success: 0.5, takesTime: false },
        memory_leak: { action: "Restart flyd", success: 0.95, takesTime: true, actionType: "RESTART_FLYD" },
        disk_io_bottleneck: { action: "Throttle I/O", success: 0.7, takesTime: false },
        kernel_panic: { action: "Reboot worker", success: 0.8, takesTime: true, actionType: "RESTART_FLYD" },
        network_congestion: { action: "Reset NIC", success: 0.6, takesTime: false },
        dns_failure: { action: "Flush DNS cache", success: 0.9, takesTime: false },
        config_corruption: { action: "Restore config", success: 0.85, takesTime: true, actionType: "RESTART_FLYD" },
        hardware_degradation: { action: "Emergency drain", success: 0.3, takesTime: true, actionType: "DRAIN_WORKER" },
        storage_spreading: { action: "Emergency drain", success: 0.4, takesTime: true, actionType: "DRAIN_WORKER" },
        network_hardware_failure: { action: "Emergency drain", success: 0.5, takesTime: true, actionType: "DRAIN_WORKER" },
      }

      const fix = quickFixes[quickFixIncident.type]
      const wasInvestigated = quickFixIncident.investigated
      const successRate = wasInvestigated ? fix.success + 0.1 : fix.success
      ...
      if (fix.actionType && quickFixIncident.workerId && fixSuccess) {
        if (fix.actionType === "DRAIN_WORKER") {
          // Directly call the drain worker logic
          const drainSuccess = Math.random() > 0.3 // 70% success rate
          ...
          if (!drainSuccess) {
            ...
            return {
              ...
              incidents: state.incidents.map((i) => (i.id === action.incidentId ? { ...i, resolved: false } : i)),
              score: {
                ...state.score,
                riskyActions: state.score.riskyActions + 1,
                failedMigrations: state.score.failedMigrations + 1,
          ...
          return {
            ...
            incidents: state.incidents.map((i) => (i.id === action.incidentId ? { ...i, resolved: true } : i)),
            score: { 
              ...state.score, 
              riskyActions: state.score.riskyActions + 1,
      ...
      return {
        ...
        incidents: state.incidents.map((i) => (i.id === action.incidentId ? { ...i, resolved: fixSuccess } : i)),
        score: {
          ...state.score,
          successfulMigrations: fixSuccess ? state.score.successfulMigrations + 1 : state.score.successfulMigrations,
          failedMigrations: fixSuccess ? state.score.failedMigrations : state.score.failedMigrations + 1,
        },
        actionFeedback: {
          type: fixSuccess ? "success" : "warning",
          title: fixSuccess ? "Quick Fix Applied" : "Quick Fix Failed",
          message: fixSuccess
            ? `Applied quick fix: ${fix.action}. ${fix.takesTime ? "Restart initiated - will take 3 seconds." : "Incident resolved immediately."} ${wasInvestigated ? "Investigation data helped improve success rate." : "Consider investigating first for better outcomes."}`
            : `Quick fix (${fix.action}) failed to resolve the incident. ${wasInvestigated ? "Try alternative approaches." : "Investigation might reveal better solutions."}`,
          timestamp: Date.now(),
        },
      }
    ...
    case "UPDATE_WORKER":
      return {
        ...state,
        workers: state.workers.map((worker) =>
          worker.id === action.workerId ? { ...worker, ...action.updates } : worker,
        ),
      }
...
export function GameProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    incidentIntervalRef.current = setInterval(() => {
      ...
      // Don't generate incidents if worker is in unstable state
      ...
      const hasMigrations = worker.activeFSMs.length > 0 && 
                           worker.activeFSMs.some(fsm => fsm.type === "migration")
      
      // Pause incident generation during certain states - including when restarting OR draining
      const shouldPauseIncidents =
        worker.flydStatus === "restarting" ||
        worker.status === "critical" ||
        hasMigrations // More reliable check for drain operations

      if (shouldPauseIncidents) {
        console.log("Pausing incidents due to worker state:", 
          hasMigrations ? "Drain in progress" : 
          worker.flydStatus === "restarting" ? "Restart in progress" : 
          "Critical state")
        return
      }

      // Higher incident rate for testing - let's make it 80% chance
      const baseIncidentRate = 0.8 // Increased for testing
      ...
      if (shouldCreateIncident) {
        ...
        // Only add flyd-related incidents if flyd is running normally
        if (worker.flydStatus === "running") {
          possibleIncidents.push(
            {
              type: "flyd_stalled" as const,
              severity: "high" as const,
              title: "flyd Process Stalled",
              description: "flyd process on worker has become unresponsive. FSM operations are backing up.",
              workerId: worker.id,
              uptime_impact: 15,
            },
            {
              ...
              description: "flyd process memory usage growing rapidly. FSM tracker leaking references.",
              workerId: worker.id,
              uptime_impact: 8,
            },
            {
              ...
              description: "flyd configuration file has become corrupted with invalid JSON syntax.",
              workerId: worker.id,
              uptime_impact: 12,
            }
          )
        ...
        if (worker.containerdHealth === "healthy") {
          possibleIncidents.push({
            ...
            description: "Lease database mismatch between flyd and containerd. Some containers in unknown state.",
            workerId: worker.id,
            uptime_impact: 10,
          })
        }
          ...
          possibleIncidents.push(
            {
              ...
              description: "Worker lost connectivity to coordination services. Local containers still running.",
              workerId: worker.id,
              uptime_impact: 14,
            },
            {
              ...
              description: "High packet loss and latency affecting machine operations and migrations.",
              workerId: worker.id,
              uptime_impact: 8,
            },
            {
              ...
              description: "NIC firmware corruption detected. Connection drops every 30-60 seconds.",
              workerId: worker.id,
              uptime_impact: 25,
              requires_drain: true,
            }
        ...
        possibleIncidents.push(
          {
            ...
            description: "LVM metadata corruption detected. Data integrity at risk.",
            workerId: worker.id,
            uptime_impact: 18,
          },
          {
            ...
            description: "LVM corruption spreading across volume group. Immediate evacuation required.",
            workerId: worker.id,
            uptime_impact: 30,
            requires_drain: true,
          }
        ...
        // Add migration-related incidents if there are active FSMs
        if (worker.activeFSMs.some(fsm => fsm.type === "migration")) {
          possibleIncidents.push({
            type: "migration_stuck" as const,
            severity: "high" as const,
            title: "Migration Stuck",
            description: "dm-clone hydration stalled at 45%. Network throughput dropped to 0 MB/s.",
            workerId: worker.id,
            uptime_impact: 12,
          })
        }
        ...
        possibleIncidents.push(
          {
            ...
            description: "ECC memory errors increasing exponentially. Hardware failure imminent.",
            workerId: worker.id,
            uptime_impact: 22,
            requires_drain: true,
          },
          {
            ...
            severity: "critical" as const,
            title: "Kernel Panic",
            description: "Worker kernel panic detected. Likely caused by faulty hardware or driver issue.",
            workerId: worker.id,
            uptime_impact: 20,
          }
        )
        ...
        possibleIncidents.push(
          {
            ...
            description: "High disk queue depth causing slow machine operations. I/O latency increasing.",
            workerId: worker.id,
            uptime_impact: 6,
          },
          {
            ...
            description: "Worker unable to resolve internal DNS names. Local resolver cache may be corrupted.",
            workerId: worker.id,
            uptime_impact: 2,
          }
        )

        console.log("Possible incidents:", possibleIncidents.length)
        ...
        const incident = possibleIncidents[Math.floor(Math.random() * possibleIncidents.length)]
        const newIncident = {
          id: `incident-${Date.now()}-${Math.random()}`,
          timestamp: Date.now(),
          resolved: false,
          isFirstTime: !state.incidents.some((i) => i.type === incident.type),
          investigated: false,
          logViewed: false,
          ...incident,
        }

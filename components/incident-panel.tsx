...
export function IncidentPanel() {
  ...
  const getIncidentDescription = (incident: any) => {
    ...
    const firstTimeDescriptions = {
      ...
      kernel_panic:
        "Kernel panics indicate serious system issues. The worker may become completely unresponsive.",
      network_congestion:
    ...
    return (
      firstTimeDescriptions[
        incident.type as keyof typeof firstTimeDescriptions
        ...
        {activeIncidents.length === 0 ? (
          ...
          activeIncidents.map((incident) => (
            <div
              ...
              onClick={() =>
                dispatch({ type: "SELECT_INCIDENT", incidentId: incident.id })
              }
            >
              ...
              {incident.workerId && (
                <div className="text-xs text-violet-400">
                  Affected:{" "}
                  {state.workers.find((w) => w.id === incident.workerId)?.name}
                </div>
              )}

              {incident.requires_drain && (
                <div className="text-xs text-red-300 bg-red-900/50 px-2 py-1 rounded mt-1">
                  <span>
              ...
              {incident.isFirstTime && (
                <div className="mt-2 p-2 bg-violet-900/50 border border-violet-500 rounded text-xs">
                  <div className="flex items-center space-x-1 text-violet-300 font-semibold mb-1">
                    <span>
                      <Lightbulb className="inline h-4 w-4" />
                    </span>
                    <span>Insights</span>
                  </div>
        ...
        {resolvedIncidents.length > 0 && (
          <div className="pt-3 border-t border-slate-700">
            ...
            {resolvedIncidents.map((incident) => (
              <div
                key={incident.id}
                className="p-2 rounded bg-slate-700/50 mb-2"
              >
